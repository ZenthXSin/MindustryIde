...
abstract class JsonEditorTool {
    ...
    fun getClassBuildByName(name: String): ClassBuild {
        return ClassBuild(getClassByName(name))
    }
    ...
    fun new(name: String, chooseClass: List<ClassData>.() -> ClassData): JsonWorkFile {
        jsonWorkFile = JsonWorkFile(name).loadClassBuild {
            val classList = Vars.parser.classMap?.map {
                if (it.key != null) {
                    ClassData(it.key!!)
                } else {
            ...
            ClassBuild(classData.classData)
        }
        return jsonWorkFile
    }
...
class ClassData(var name: String, var doc: String = "", var classData: Class<*> = Nullable::class.java) {
    init {
        classData = Vars.parser.classMap?.get(name) ?: Nullable::class.java
        doc = Vars.parser.getClassDoc(name)
    }
