# arc:arc-core/src/arc/struct/LongMap.java

> Source: `/home/zenxsin/cow/源码和示例/Arc/arc-core/src/arc/struct/LongMap.java`

## 源码位置

- 仓库：`arc`
- 相对路径：`arc-core/src/arc/struct/LongMap.java`
- 绝对路径：`/home/zenxsin/cow/源码和示例/Arc/arc-core/src/arc/struct/LongMap.java`
- SHA1：`98726e9d2f1f5e3824f8c1d61293e104e34af2c4`
- 大小：29120 bytes
- 行数：893

## 文件职责摘要

此页是文件级知识点。当前静态摘要：`class LongMap<V> implements Iterable<LongMap.Entry<V>>`。

关键词：arc

## Package

`arc.struct`

## 类型声明

- `class LongMap<V> implements Iterable<LongMap.Entry<V>>`
- `class Entry<V>`
- `class MapIterator<V>`
- `class Entries<V> extends MapIterator<V> implements Iterable<Entry<V>>, Iterator<Entry<V>>`
- `class Values<V> extends MapIterator<V> implements Iterable<V>, Iterator<V>`
- `class Keysextends MapIterator`

## 字段线索

- `PRIME1`
- `PRIME2`
- `PRIME3`
- `EMPTY`
- `size`
- `loadFactor`
- `threshold`
- `stashCapacity`
- `pushIterations`
- `entries2`
- `values2`
- `keys2`
- `INDEX_ZERO`
- `key`
- `value`
- `INDEX_ILLEGAL`
- `map`
- `hasNext`
- `entry`

## 方法线索

- `IllegalArgumentException`
- `put`
- `putAll`
- `putResize`
- `push`
- `putStash`
- `get`
- `getStash`
- `remove`
- `isEmpty`
- `shrink`
- `clear`
- `containsValue`
- `containsKey`
- `containsKeyStash`
- `findKey`
- `ensureCapacity`
- `resize`
- `hash2`
- `hash3`
- `hashCode`
- `equals`
- `toString`
- `iterator`
- `entries`
- `values`
- `eachValue`
- `keys`
- `reset`
- `IllegalStateException`

## 使用方式

- 需要全文时，直接读取上方 Source 路径对应的本地源码文件。
- 本页保留源码位置、hash 和静态分析结果，不复制源码全文。
- 总索引见 [overview](../overview.md)。

## Related

- [Mindustry / Arc Full Source Knowledge Index Plan](../../mindustry-source-index/overview.md)
- [Mindustry & Arc 源码位置](../../mindustry/source-code-locations.md)
